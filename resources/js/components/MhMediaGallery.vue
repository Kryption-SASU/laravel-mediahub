...
<template>
    <div :class="cls('root')" role="group" :aria-labelledby="label ? labelId : undefined">
        ...
        <p v-if="items.length === 0" :class="cls('empty')">{{ emptyLabel }}</p>
        ...
        <ol v-else :class="cls('list')">
            <li v-for="(item, index) in items" :key="item.id" :class="cls('item')">
                ...
                <button
                    ...
                    :aria-label="`${moveUpLabel}: ${item.media?.name ?? item.id}`"
                    @click="move(index, -1)"
                >
                    {{ moveUpLabel }}
                </button>
                ...
                <button
                    ...
                    :aria-label="`${moveDownLabel}: ${item.media?.name ?? item.id}`"
                    @click="move(index, 1)"
                >
                    {{ moveDownLabel }}
                </button>
                ...
                <button
                    ...
                    :class="cls('remove')"
                    :disabled="disabled"
                    :aria-label="`${removeLabel}: ${item.media?.name ?? item.id}`"
